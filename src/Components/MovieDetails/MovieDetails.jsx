  ...
  return (
    ...
    {state.length !==0 && <>
      ...
      <div className="moviePoster">
        <img className='moviedetailImage' src={`https://image.tmdb.org/t/p/original/${state && state.backdrop_path}`} alt=""/>
       
        <div className="innermoviePoster">
        <h2 className="movieTitle">{state && state.title}</h2>
        <button onClick={() => {
              setVideodiv(true)
            }} className='trailer'><i className="fa-solid fa-play"></i> Watch Trailer</button>
        </div>
      ...
      <div className="finalTrial">
        ...
        />
          ...
          <div className="movieFullDetails">
            {/* <h2 className="movieTitle">{state && state.title}</h2> */}
            <p>
              Status: <span> {state && state.status} {state.status === "Released" ? <i class="cross fa-solid fa-circle-check"></i> : <i class=" fa-sharp fa-regular fa-circle-xmark"></i>}</span>
            </p>
            <p>
              Release Date: <span> {state && state.release_date}</span>{" "}
            </p>
            <p>
              RumTime: <span> {state && state.runtime} min <i class="clock fa-solid fa-clock"></i></span>
            </p>
            ...
            <p>
              Rating: <span> {state && state.vote_average.toFixed(1)}</span>{" "}<i className="rating fa-brands fa-imdb"></i>
            </p>
            <p>
        ...
        <div className="castDetails">
          {cast && cast.map((cast) => {
            return (
              ...
              <div className="cast">
                <img src={`https://image.tmdb.org/t/p/original/${cast && cast.profile_path}`} alt="loading" />
